...
import { PlusCircle, Timer, Repeat, Dumbbell, MoreHorizontal, Trash2 } from 'lucide-react'
...
const SeriesForm: React.FC<SeriesFormProps> = ({ series, onChange, onDelete, canDelete }) => {
    ...
    const handleRepChange = (updatedRep: Rep) => {
        onChange({
            ...series,
            reps: series.reps.map(rep => (rep.id === updatedRep.id ? updatedRep : rep)),
        })
    }
        ...
        <div className='space-y-4 pb-2'>
                ...
                <div>
                    ...
                    <div className='flex items-center gap-2'>
                        <Input
                            ...
                            className='h-8'
                        />
                        <span className='text-xs text-muted-foreground w-12'>{formatTime(series.restSeries)}</span>
                    </div>
                </div>
            </div>
            ...
            <div>
                <div className='flex justify-between items-center mb-2'>
                    ...
                    {canDelete && (
                        <Button variant='ghost' size='sm' onClick={onDelete} className='h-7 hover:text-destructive'>
                            <Trash2 size={14} />
                        </Button>
                    )}
                </div>
                ...
                <Button variant='outline' size='sm' onClick={handleAddRep} className='w-full mt-2 text-xs h-8'>
    ...
    return (
        <AccordionItem value={`rep-${index}`} className='border-0 w-full'>
            <div className='flex items-center justify-between w-full'>
                ...
                {canDelete && (
                    <div>
                        <Button variant='ghost' size='sm' onClick={handleDeleteClick} className='h-7 hover:text-destructive ml-2'>
                            <Trash2 size={14} />
                        </Button>
                    </div>
                )}
                ...
                <div className='space-y-3'>
                    ...
                    <div>
                        <div className='flex justify-between items-center mb-1'>
                            ...
                            <span className='text-xs text-muted-foreground'>{formatTime(rep.repDuration)}</span>
                        </div>
                        <Slider
                    ...
                    <div>
                        <div className='flex justify-between items-center mb-1'>
                            ...
                            <span className='text-xs text-muted-foreground'>{formatTime(rep.restDuration)}</span>
                        </div>
                        <Slider
